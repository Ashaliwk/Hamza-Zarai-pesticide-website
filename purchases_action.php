...
if ($action === 'add') {
    ...
    $stmt = $pdo->prepare("INSERT INTO purchases (product_id, supplier_name, quantity, price_per_unit, total, purchase_date) VALUES (?,?,?,?,?,?)");
    $stmt->execute([$product_id, $supplier, $qty, $price, $total, $date]);
    ...
    if ($product_id === $old_product_id) {
        ...
        $updPur = $pdo->prepare("UPDATE purchases SET product_id = ?, supplier_name = ?, quantity = ?, price_per_unit = ?, total = ?, purchase_date = ? WHERE id = ?");
        $updPur->execute([$product_id, $supplier, $qty, $price, $total, $date, $id]);
        $pdo->commit();
    } else {
        $pdo->beginTransaction();
